...
async function renderFirstChart() {
    ...
    const data = await d3.csv("https://jasondlundquist.github.io/data/hours-vs-gdp.csv");
    const year = 2015
...
function renderFirstChartAnnotations(d, x, y, margin) {
    const computedDX = d.entity == "United Kingdom" ? -30 : 30;
    const computedDY = d.entity == "United Kingdom" ? 30 : -30;
    const annotations = [
        {
            note: {
...
async function renderSecondChart() {
    const margin = {top: 10, right: 20, bottom: 30, left: 50},
        width = 1000 - margin.left - margin.right,
        height = 800 - margin.top - margin.bottom;
    const data = await d3.csv("https://jasondlundquist.github.io/data/productivity-vs-hours-worked.csv");
    const year = 2015
    const filteredData = data.filter(function (d) {
...
async function renderThirdChart() {
    ...
    const data = await d3.csv("https://jasondlundquist.github.io/data/productivity-PennWorldTable.csv");
...
// Load external data
    Promise.all([
        ...
        d3.csv("https://jasondlundquist.github.io/data/hours-vs-gdp.csv", function (d) {
            if (d.year == 2015) {
                data.set(d.code,
                    {
                        year: d.year,
